...
public fun <V> Activity.getExtra(key: String, fallback: V? = null): V =
        getExtraOptional(key, fallback)!!

public fun <V> Activity.getExtraOptional(key: String, fallback: V? = null): V? =
        intent?.extras?.get(key) as? V ?: fallback

public fun <V> Fragment.getExtra(key: String, fallback: V? = null, checkActivity: Boolean = false): V =
        getExtraOptional(key, fallback, checkActivity)!!

public fun <V> Fragment.getExtraOptional(key: String, fallback: V? = null, checkActivity: Boolean = false): V? {
    var value = arguments?.get(key) as? V
    if (value == null && checkActivity) {
        value = activity?.getExtraOptional<V>(key, null)
    }
...
public fun <V> Activity.bindExtra(key: String, fallback: V? = null): kotlin.Lazy<V> =
        lazy { getExtraOptional(key, fallback)!! }

public fun <V> Activity.bindExtraOptional(key: String, fallback: V? = null): kotlin.Lazy<V?> =
        lazy { getExtraOptional(key, fallback) }

public fun <V> Fragment.bindExtra(key: String, fallback: V? = null, checkActivity: Boolean = false): kotlin.Lazy<V> =
        lazy { getExtraOptional(key, fallback, checkActivity)!! }

public fun <V> Fragment.bindExtraOptional(key: String, fallback: V? = null, checkActivity: Boolean = false): kotlin.Lazy<V?> =
        lazy { getExtraOptional(key, fallback, checkActivity) }
...
public fun <V : View> android.app.Fragment.bindView(id: Int)
        : ReadOnlyProperty<android.app.Fragment, V> = required(id, viewFinder)
...
public fun <V : View> android.app.Fragment.bindOptionalView(id: Int)
        : ReadOnlyProperty<android.app.Fragment, V?> = optional(id, viewFinder)
...
public fun <V : View> android.app.Fragment.bindViews(vararg ids: Int)
        : ReadOnlyProperty<android.app.Fragment, List<V>> = required(ids, viewFinder)
...
public fun <V : View> android.app.Fragment.bindOptionalViews(vararg ids: Int)
        : ReadOnlyProperty<android.app.Fragment, List<V>> = optional(ids, viewFinder)
...
private val android.support.v4.app.DialogFragment.viewFinder: android.support.v4.app.DialogFragment.(Int) -> View?
    get() = { dialog?.findViewById(it) ?: view?.findViewById(it) }
private val android.app.Fragment.viewFinder: android.app.Fragment.(Int) -> View?
